...
def read_geojson(file):
    gdf = None  # Initialize the variable
    if file is not None:
        try:
...
def main():
    ...
    st.sidebar.header("Upload ROI as JSON")
    json_file = st.sidebar.file_uploader("Upload a GeoJSON", type=["geojson"])
    ...
    with col2:
        ...
        #     custom_geojson = read_geojson(json_file)
        
        # else:
        # User input for GeoJSON coordinates
        custom_geojson_json = st.text_area("Enter your GeoJSON here as JSON for the ROI:", "")

        # Check if the user input is empty
        if not custom_geojson_json:
            st.error("Please add a GeoJSON in JSON format.")
        else:
            # Attempt to load the GeoJSON
            try:
                custom_geojson = json.loads(custom_geojson_json)
                st.success("GeoJSON loaded successfully.")
                # Further processing with the loaded GeoJSON can be added here
        ...
        st.write(f'Number of total satellite images in the range provided: {image_count}')
        ...
        with col1:
            ...
            # # Specify the download path as an absolute path
            # download_path = os.path.expanduser('~/Downloads')

            # # Create a temporary directory
            # temp_dir = tempfile.TemporaryDirectory()

            # if st.button("Export ROI as Shapefile"):
            #     try:
            #         # Define the output shapefile name
            #         output_shp = "roi_shapefile.shp"

            #         # Use geemap to export the ROI as a shapefile and package it into a zip folder
            #         exported_file = geemap.ee_export_vector(roi_fc, output_shp, out_dir=temp_dir.name)

            #         # Ensure the download path exists and create it if not
            #         if not os.path.exists(download_path):
            #             os.makedirs(download_path)

            #         # Move the exported file to the user's Downloads directory
            #         exported_file_path = os.path.join(download_path, "roi_shapefile.zip")
            #         shutil.move(exported_file, exported_file_path)

            #         # Generate a downloadable URL for the file
            #         download_url = f"[Download ROI Shapefile]({exported_file_path})"

            #         # Clean up the temporary directory
            #         temp_dir.cleanup()

            #         # Display the download link in Streamlit
            #         st.markdown(download_url)
        ...
        with col2:
            ...
            st.write("Satellite imagery datetimes:")
