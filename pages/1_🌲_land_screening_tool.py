...
with col1:
    ...
    if custom_geojson_json:
        ...
        st.warning("Enter your GeoJSON in the text area above and press 'Ctrl' + 'Enter'.")
...
if st.button("Calculate NLCD") and roi_geometry:
    ...
    with col1:
        ...
        st.write(f"Total Plantable Area (non-forested) within {slope_threshold}% slope threshold: {total_area_nf:.2f} Sq. Km")
    ...
    m3.addLayer(L8_collection.median().clip(roi), {  
        'bands': ['B4', 'B3', 'B2'],
        'min': 0,
        'max': 3000,
        'gamma': 1.4
...
st.experimental_set_query_params(cleanup=temp_dir.name)